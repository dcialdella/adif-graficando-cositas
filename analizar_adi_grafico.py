...
def parse_adi_file(filename):
    ...
    # Separar por registros QSO (EOR = End Of Record) - case insensitive
...
def _normalize_band(band):
    """Normaliza nombre de banda a formato estándar (ej: 20M, 40M, 2M)."""
    if not band:
        return "Unknown"
    band = band.upper().strip()
    if band and band[-1].isdigit():
        band += "M"
    return band
...
def create_locators_chart(locators_data):
    ...
    bars = ax.bar(
        locators, counts, color="lightgreen", edgecolor="darkgreen", alpha=0.7
    )
...
def create_mode_band_chart(mode_band_data):
    ...
    bars2 = ax2.bar(
        bands, band_counts, color="lightblue", edgecolor="darkblue", alpha=0.7
    )
...
def create_stations_chart(stations_data):
    ...
    ax.set_title(
        "Top 20 Estaciones por Número de Contactos", fontsize=14, fontweight="bold"
    )
...
def create_time_distribution_chart(time_data):
    ...
    ax1.set_title(
        "Distribución de Contactos por Hora UTC", fontsize=14, fontweight="bold"
    )
...
def create_world_map(qsos):
    ...
    try:
        ...
        ax.set_title(
            "Distribución Mundial de Localizadores Maidenhead",
            fontsize=14,
            fontweight="bold",
        )
...
def create_distance_histogram(qsos):
    ...
    log_bins = np.logspace(
        np.log10(max(1, min(distances))), np.log10(max(distances) + 1), 20
    )
    ...
    ax2.set_title(
        "Distribución de Distancias (Escala Log)", fontsize=14, fontweight="bold"
    )
    ax2.set_xscale("log")
...
def create_distance_by_locator_chart(qsos):
        ...
        for mode in modes:
            ...
            if points:
                ...
                ax3.scatter(
                    [band] * len(distances),
                    distances,
                    c=[mode_to_color[mode]] * len(distances),
                    s=[p / 2 for p in powers],
                    alpha=0.6,
                    label=f"{mode}" if band == bands[0] else "",
                    marker="o",
                )
    ...
    ax3.set_title(
        "Distancia por Banda (color=Modo, tamaño=Potencia)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    bp = ax4.boxplot(box_data, label=box_labels, patch_artist=True)
    ...
    ax4.set_title(
        "Distribución de Distancias por Banda", fontsize=14, fontweight="bold"
    )
...
def create_summary_dashboard(qsos):
    ...
    fig.suptitle(
        f"Dashboard Resumen - Total: {len(qsos)} QSOs", fontsize=16, fontweight="bold"
    )
...
def create_qrz_lookups_chart():
    ...
    sorted_data = sorted(
        lookups_data.items(), key=lambda x: x[1]["lookups"], reverse=True
    )
...
def create_fonia_por_hora_chart(qsos):
    ...
    ax1.set_title(
        "Contactos en Fonía (SSB/FM) por Hora UTC", fontsize=14, fontweight="bold"
    )
    ax1.set_xticks(hours)
    ax1.set_xticklabels([f"{h:02d}:00" for h in hours], rotation=45)
    ...
    ax2.set_title(
        "Distribución Horaria de Contactos en Fonía", fontsize=14, fontweight="bold"
    )
    ax2.set_xticks(hours)
    ax2.set_xticklabels([f"{h:02d}:00" for h in hours], rotation=45)
    ax2.grid(True, alpha=0.3)
...
# ==============================================================================
# SECCIÓN 4: NUEVOS GRÁFICOS
# ==============================================================================


def create_band_hour_heatmap(qsos):
    """
    Genera heatmap de bandas vs hora UTC.

    Muestra en un mapa de calor qué bandas se utilizan en cada hora del día,
    permitiendo identificar patrones de propagación y horarios de operación.

    Args:
        qsos (list): Lista de diccionarios de QSOs
    """
    band_hour = defaultdict(lambda: defaultdict(int))
    all_bands = set()

    for qso in qsos:
        band = qso.get("BAND", "")
        time_str = qso.get("TIME_ON", "")
        if band and time_str and len(time_str) >= 2:
            band = _normalize_band(band)
            try:
                hour = int(time_str[:2])
                band_hour[band][hour] += 1
                all_bands.add(band)
            except ValueError:
                continue

    if not all_bands:
        print("  (sin datos para heatmap banda-hora)")
        return

    bands_sorted = sorted(
        all_bands, key=lambda b: sum(band_hour[b].values()), reverse=True
    )

    matrix = np.zeros((len(bands_sorted), 24))
    for i, band in enumerate(bands_sorted):
        for h in range(24):
            matrix[i][h] = band_hour[band][h]
    ...
    ax1.set_yticks(range(len(bands_sorted)))
    ax1.set_yticklabels(bands_sorted)
    ax1.set_xlabel("Hora (UTC)", fontsize=12)
    ax1.set_ylabel("Banda", fontsize=12)
    ax1.set_title("Heatmap: Banda vs Hora UTC", fontsize=14, fontweight="bold")
    ...
    for i in range(len(bands_sorted)):
        for j in range(24):
            ...
            if val > 0:
                color = "white" if matrix[i][j] > matrix.max() / 2 else "black"
                ax1.text(j, i, str(val), ha="center", va="center", color=color, fontsize=7)

    band_totals = [sum(band_hour[b].values()) for b in bands_sorted]
    bars = ax2.bar(bands_sorted, band_totals, color="steelblue", edgecolor="navy", alpha=0.7)
    ax2.set_xlabel("Banda", fontsize=12)
    ax2.set_ylabel("Total QSOs", fontsize=12)
    ax2.set_title("Total de QSOs por Banda", fontsize=12, fontweight="bold")
    for bar, total in zip(bars, band_totals):
        ax2.text(
    ...
    plt.tight_layout()
    plt.savefig("grafico_banda_hora.png", dpi=300, bbox_inches="tight")
    plt.close()


def create_unique_stations_chart(qsos):
    """
    Genera gráfico donut de estaciones únicas vs repetidas.

    Muestra qué porcentaje de estaciones se contactaron solo una vez
    versus las que se repitieron, indicando la diversidad del log.

    Args:
        qsos (list): Lista de diccionarios de QSOs
    """
    calls = [qso.get("CALL", "") for qso in qsos if qso.get("CALL")]
    if not calls:
        return

    call_counts = Counter(calls)
    total_stations = len(call_counts)
    unique_stations = sum(1 for c in call_counts.values() if c == 1)
    repeated_stations = sum(1 for c in call_counts.values() if c > 1)

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))

    sizes = [unique_stations, repeated_stations]
    labels = [
        f"Únicas (1 QSO)\n{unique_stations}",
        f"Repetidas (>1 QSO)\n{repeated_stations}",
    ]
    ...
    ax1.pie(
        ...
        autopct=lambda p: f"{p:.1f}%\n({int(p/100*total_stations)})",
        startangle=90, pctdistance=0.75,
        wedgeprops={"edgecolor": "white", "linewidth": 2},
    )
    centre_circle = plt.Circle((0, 0), 0.50, fc="white")
    ax1.add_artist(centre_circle)
    ax1.set_title("Estaciones Únicas vs Repetidas", fontsize=14, fontweight="bold")

    repeated = {k: v for k, v in call_counts.items() if v > 1}
    top_repeated = dict(sorted(repeated.items(), key=lambda x: x[1], reverse=True)[:10])

    if top_repeated:
        stations_list = list(top_repeated.keys())
        counts_list = list(top_repeated.values())
        bars = ax2.barh(
            stations_list, counts_list, color="#fc8d62", edgecolor="black", alpha=0.8
        )
        ax2.set_xlabel("Número de QSOs", fontsize=12)
        ax2.set_title("Top 10 Estaciones Repetidas", fontsize=13, fontweight="bold")
        ax2.invert_yaxis()
    ...
    fig.suptitle(
        f"Total estaciones únicas: {total_stations} | "
        f"Únicas: {unique_stations} ({unique_stations/total_stations*100:.1f}%) | "
        f"Repetidas: {repeated_stations} ({repeated_stations/total_stations*100:.1f}%)",
        fontsize=11, y=1.02,
    )

    plt.tight_layout()
    plt.savefig("grafico_estaciones_unicas.png", dpi=300, bbox_inches="tight")
    plt.close()


def create_world_map_by_operator(qsos):
    """
    Genera mapa mundial con puntos coloreados por operador.

    Cada operador aparece en un color distinto, permitiendo visualizar
    la distribución geográfica de contactos de cada operador.

    Args:
        qsos (list): Lista de diccionarios de QSOs
    """
    try:
        op_locs = defaultdict(list)

        for qso in qsos:
            loc = qso.get("GRIDSQUARE", "")
            operator = qso.get("OPERATOR", "Unknown") or "Unknown"
            if loc and len(loc) >= 4:
                loc_main = loc[:6] if len(loc) >= 6 else loc[:4]
                lat, lon = maidenhead_to_latlon(loc_main)
                if lat is not None and lon is not None:
                    op_locs[operator].append((lat, lon))

        if not op_locs:
            print("  (sin datos de localizadores para mapa por operador)")
            return

        fig, ax = plt.subplots(figsize=(16, 10))

        operators = sorted(op_locs.keys())
        colors = plt.cm.tab10(np.linspace(0, 1, len(operators)))

        for i, (op, points) in enumerate(op_locs.items()):
            lats = [p[0] for p in points]
            lons = [p[1] for p in points]
            ax.scatter(lons, lats, c=[colors[i]], label=op, s=30,
        ...
        ax.set_ylabel("Latitud", fontsize=12)
        ax.set_title(
            "Distribución Mundial por Operador", fontsize=14, fontweight="bold"
        )
        ax.grid(True, alpha=0.3)
        ax.axhline(y=0, color="gray", linestyle="--", alpha=0.5)
        ax.axvline(x=0, color="gray", linestyle="--", alpha=0.5)
        ax.legend(title="Operador", bbox_to_anchor=(1.02, 1), loc="upper left")

        plt.tight_layout()
        plt.savefig("grafico_mapa_operadores.png", dpi=300, bbox_inches="tight")
        plt.close()

    except Exception as e:
        print(f"  (mapa por operador omitido: {e})")


def create_sankey_diagram(qsos):
    """
    Genera diagrama Sankey del flujo: Operador -> Modo -> Banda.

    El Sankey se guarda como HTML interactivo usando Plotly.
    ...
    try:
        ...
        print("  (Sankey: instala plotly con: pip install plotly)")
        return

    triple_counts = Counter()
    for qso in qsos:
        op = qso.get("OPERATOR", "Unknown") or "Unknown"
        mode = qso.get("MODE", "Unknown") or "Unknown"
        band = qso.get("BAND", "Unknown") or "Unknown"
        band = _normalize_band(band)
        triple_counts[(str(op), str(mode), str(band))] += 1

    operators = sorted(set(t[0] for t in triple_counts))
    modes = sorted(set(t[1] for t in triple_counts))
    bands = sorted(set(t[2] for t in triple_counts))

    all_nodes = operators + modes + bands
    node_to_idx = {name: i for i, name in enumerate(all_nodes)}
    op_colors_cmap = plt.cm.Set1(np.linspace(0, 1, len(operators)))
    mode_colors_cmap = plt.cm.Set2(np.linspace(0, 1, len(modes)))
    band_colors_cmap = plt.cm.Set3(np.linspace(0, 1, len(bands)))

    def _rgba(c, a=0.85):
        return f"rgba({int(c[0]*255)},{int(c[1]*255)},{int(c[2]*255)},{a})"

    node_colors = (
        [_rgba(op_colors_cmap[i]) for i in range(len(operators))]
        + [_rgba(mode_colors_cmap[i]) for i in range(len(modes))]
        + [_rgba(band_colors_cmap[i]) for i in range(len(bands))]
    )
    ...
    for op in operators:
        for mode in modes:
            v = sum(c for (o, m, b), c in triple_counts.items() if o == op and m == mode)
            if v > 0:
                sources.append(node_to_idx[op])
                targets.append(node_to_idx[mode])
                values.append(v)
                link_labels.append(f"{v}")

    for mode in modes:
        for band in bands:
            v = sum(c for (o, m, b), c in triple_counts.items() if m == mode and b == band)
            if v > 0:
                sources.append(node_to_idx[mode])
                targets.append(node_to_idx[band])
                values.append(v)
                link_labels.append(f"{v}")
    ...
    fig.update_layout(
        title_text="Flujo: Operador \u2192 Modo \u2192 Banda",
        font_size=12, height=700, width=1200,
    )

    pio.write_html(fig, file="grafico_sankey.html", auto_open=False)
...
def generate_statistics_report(qsos):
    ...
    if cq_zones:
        print(
            f"  ✓ grafico_zonas.png ({len(cq_zones)} zonas CQ, {len(itu_zones)} zonas ITU)"
        )
    ...
    if locator_dist_data:
        print(
            f"  ✓ grafico_distancia_locator.png ({locator_dist_data['total_qsos']} QSOs, dist avg: {locator_dist_data['avg_distance']:.0f} km)"
        )
    else:
    ...
    print(
        f"  ✓ grafico_fonia_por_hora.png ({fonia_data['total']} QSOs fonía, pico: {fonia_data['hora_pico']:02d}:00 UTC)"
    )

    # --- NUEVOS GRÁFICOS ---
    print("\nGenerando nuevos gráficos...")

    create_band_hour_heatmap(qsos)
    print("  ✓ grafico_banda_hora.png")

    create_unique_stations_chart(qsos)
    print("  ✓ grafico_estaciones_unicas.png")

    create_world_map_by_operator(qsos)
    print("  ✓ grafico_mapa_operadores.png")

    create_sankey_diagram(qsos)
    print("  ✓ grafico_sankey.html (diagrama Sankey interactivo)")
...
def main():
    ...
    print("\n  NUEVOS GRÁFICOS:")
    print("    - grafico_banda_hora.png (heatmap banda vs hora)")
    print("    - grafico_estaciones_unicas.png (únicas vs repetidas)")
    print("    - grafico_mapa_operadores.png (mapa por operador)")
    print("    - grafico_sankey.html (diagrama Sankey interactivo)")
    print("=" * 80)
